...
class Laporan extends CI_Controller {
	...
	public function index()
	{
        ...
        $nama = $this->input->post('donaturs');
        $sampai = $this->input->post('tanggalak');
	$lap = $this->db->query('SELECT di.entri_tgl as tanggal, dnt.noid, left(dnt.nama,14) as nama, prg.NM_PROGRAM as program, di.besar as jumlah from donaturbaru td
	...
	if($lap->num_rows==0){
        ...
        left join donatur_item di on di.noid=td.autoid 
        left join program prg on prg.PROG=di.prog 
        where dnt.tglm between "'.$dari.'" and "'.$sampai.'" and td.nama ="'.$nama.'"');
	...
	?>
        <head>
        <title>Bukti Pembayaran Program AMAL YDSF</title>
         <img style="margin-right:1070px;width: 265px" src="<?php echo base_url('/assets/logo.png')?>">
        </head>
