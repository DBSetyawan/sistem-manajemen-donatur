...
<div class="tab-content">
  ...
  <div id="tbhdonatur" class="tab-pane fade in active">
    <br>
    <?php $this->load->view('modal_donaturs');?>
  </div>
</div>
</div>
...
<!-- Transaksi Laporan -->
  <div class="container">
    <ul class="nav nav-tabs">
  <li class="active">
      <a data-toggle="tab" id="Laporan">Laporan Pendonatur</a>
  </li>
</ul>
<div class="tab-content">
...
<!-- Transaksi Laporan Pengajuan Acara-->
  <div class="container">
    <ul class="nav nav-tabs">
  <li class="active">
      <a data-toggle="tab" id="Laporanacara">Laporan Dana Pengajuan Acara</a>
  </li>
</ul>
<div class="tab-content">
...
<div class="tab-content">
  <div id="sdcv" class="tab-pane fade">
    ...
    <table id="mntngpb" class="table table-striped table-bordered" cellspacing="0" width="100%">
          ...
          <a href="http://localhost/phpmyadmin/sql.php?db=yayasan_donatur&token=c8c4fa4d677885afae1e93efced2297a&goto=db_structure.php&table=prbh_dnt&pos=0" title="Log out" target="_blank" style="font-family:Bradley hand ITC" >
                               Admin Panel DNT_NEW >>
                            </a>
  </div>
  <div id="sdab" class="tab-pane fade in active">
     <table id="mntgdnt" class="table table-striped table-bordered" cellspacing="0" width="100%">
         ...
         <a href="http://localhost/phpmyadmin/sql.php?db=yayasan_donatur&token=c8c4fa4d677885afae1e93efced2297a&goto=db_structure.php&table=prbh_dnt_item&pos=0" title="Log out" target="_blank" style="font-family:Bradley hand ITC" >
                               Admin Panel DNT_ITEM >>
                            </a>
  </div>
</div>
</div>
...
$(".glyphicon-eye-open").mousedown(function(){
        ...
        $(function(){
            $('.autocomplete').autocomplete({
                ...
                serviceUrl: site+'/ydsfku/searchs',
...
$(document).ready(function() {
  $("#form").validate({
    ...
    messages: {
      ...
      jupen: "Jupen tidak boleh kosong",
      tempatlahir: "maaf tidak boleh kosong",
...
/*if(tempatlahir != ' ' || tgllahir != ' ' || telepon != ' ' || nohp != ' ' || nohp3 != ' ' || teleponktr != ' ' || faxktr != ' ' || tlprmh != ' ' || email != ' ' || carabayar != ' ' || rekeningdonatur != ' ' || bank != ' ' || kolektif != ' ' || waktutgh != ' ' || npwp != ' ' || info != ' ' || worker != ' ' || jabatan != ' ' || hobi != ' ' || pendidikan != ' ' || salary != ' ' || keterangan != '')*/
    ...
    if (tgllahir == '' || email == '' || carabayar == '00' || bank == '00' || waktutgh == '00' ||  info == '00' || worker == '00' || jabatan == '00' || hobi == '00' || pendidikan == '00' || salary == '00') {
...
function save_progrm()
{
    ...
    var namaprog = $("#programs1").val();
    var hargag = $("#harga1").val();
    var kelompk = $("#kelompok1").val();

    url = "<?php echo site_url('ydsfku/ajax_add_programs')?>";
    // ajax adding data to database

    if (namaprog == '' || hargag == '' || kelompk == '') {
      alertify.alert("Form ini tidak boleh kosong");
...
function save_kgts()
{
    ...
    $.ajax({
        ...
        alertify.alert("Gagal menyimpan kedalam database. <br /> Cek Kembali Nama ketua pelaksana. <br /> kemungkinan tidak ada dalam database, <br /> Terima Kasih.", function (e) {
        alertify.set({ buttonReverse: true });
        alertify.set({ buttonFocus: "ok" });
...
{ 
    ...
    url = "<?php echo site_url('ydsfku/ajax_add_secusers')?>";
    ...
    $.ajax({
       ...
       /* alertify.alert("Gagal menyimpan kedalam database.", function (e) {
        ...
        alertify.set({ buttonFocus: "ok" });*/
            ...
            /*if (e) {*/
               alertify.alert("Peringatan!!! <br> Username dan email pastikan belum ada di database")
                //on click ok, href="?"
           /* } else{
                //on click cancel, href="?"
            }*/
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            {                
                ...
                //$('#modal_donatur').modal('hide');
                //reload_table();ydsfku                //$(location).attr('href', 'formdonatur')
  ...
  function(isConfirm){
    ...
    if (nominal == '0' || nominal == '00' || nominal == '000' || nominal == '0000' || nominal == '00000' || nominal == '000000' || nominal == '0000000' || nominal == '000000000' || nominal == '00000000' || nominal == '0000000000' || nominal == '00000000000' || nominal == '00000000000' || nominal == '00000000000' || nominal == '000000000000' || nominal == '0000000000000' || nominal == '00000000000000' || nominal == '000000000000000' )  {
          ...
          $.ajax({
              ...
              error: function (jqXHR, textStatus, errorThrown)
              {
                  alertify.alert("Sistem menemukan program yang sama, <br> Silakan update ulang program sebelumnya. <br> Terima kasih.", function (e) {
                  alertify.set({ buttonReverse: true });
                  alertify.set({ buttonFocus: "ok" });
...
function save_donatur()
{
    ...
    if(save_method =='update') {
        url = "<?php echo site_url('ydsfku/ajax_update')?>";
          ...
          alertify.alert("Email tidak valid.");
          $('#update').text('Simpan'); //change button text
          $('#update').attr('disabled',false); //set button disable 
    } else {
...
function save_secusers_updates()
{
    ...
    var dotpos = email.lastIndexOf(".");
      ...
      if(atpos < 1 || dotpos < atpos+2 || dotpos+2 >= email.length) {
          alertify.alert("Email yang anda ubah tidak valid.");
          $('#ubahfusrs').text('Ubah'); //change button text
          $('#ubahfusrs').attr('disabled',false); //set button disable 
        } else {
    ...
    if(save_method =='update') {
        url = "<?php echo site_url('ydsfku/ajax_update_secusers')?>";      
    } else {
       url = "<?php echo site_url('ydsfku/ajax_edit_secusers')?>";
    }
...
<div class="copyright pull-center">
    <img style="padding-bottom:100px;margin-left:600px;width:270px" src="<?php echo base_url('/assets/logo.png')?>">
</div>
<hr>
<center>
<footer class="footer">
